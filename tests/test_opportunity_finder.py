...
class TestEvaluateStrategy:
    ...
    def test_spread_below_min_skipped(self):
        # 99.5c combined → 0.5c spread < 0.8c min
        pair = _make_crypto_pair()
        opp = _evaluate_strategy(pair, 59.5, 40.0, "YES", "NO")
        assert opp is None

    def test_hours_to_close_uses_earlier_time(self):
        pair = _make_crypto_pair(hours=48.0)
        opp = _evaluate_strategy(pair, 50.0, 40.0, "YES", "NO")
        assert opp is not None
...
class TestOpportunityFinder:
    ...
    def test_crypto_strategy_a_found(self):
        # K-YES=57, P-NO=40 → 97c → spread 3c
        pair = _make_crypto_pair(k_yes_ask=57.0, p_no_ask=40.0)
        ...
        assert strat_a[0].spread_cents == 3.0
    ...
    def test_both_strategies_found(self):
        # A: 57+40=97c (3c), B: 45+42=87c (13c) — both profitable
        pair = _make_crypto_pair(k_yes_ask=57.0, k_no_ask=45.0, p_yes_ask=42.0, p_no_ask=40.0)
        opps = self.finder.find_opportunities([pair])
        assert len(opps) == 2
    ...
    def test_sorted_best_spread_first(self):
        ...
        pair2 = _make_crypto_pair(k_yes_ask=57.0, p_no_ask=40.0)    # A: 97c → spread=3c
...
class TestFormatOpportunityLogCrypto:
    def test_contains_key_fields(self):
        ...
        assert "57.0c" in text
        assert "40.0c" in text
        assert "97.0c" in text
